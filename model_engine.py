...
# ---------------------------------------------------------------------------
# CE-18 v4.3: paper-specific necessary evidence gates
# ---------------------------------------------------------------------------
#
...
PROPERTY_EVIDENCE_GATES = {
    ...
    "Availability": [
        # Availability is about loss of timely access to required resources/services.
        # Generic "disrupted operations" or physical-process disruption alone is NOT enough.
        r"\b(?:outage|unavailable|inaccessible|offline|lockout|service disruption|service interruption|loss of access|lost access)\b",
        r"\b(?:shut down|shutdown|disabled|interrupted)\b.{0,80}\b(?:service|system|network|controller|production|power delivery|communication)\b",
        r"\b(?:service|system|network|controller|production|power delivery|communication)\b.{0,80}\b(?:unavailable|offline|inaccessible|interrupted|shut down)\b",
...
def classify_incident(
    ...
    """
    CE-18 v4.3: Paper-Constrained Contrastive Entailment.
    ...
    for prop in PROPERTIES:
        ...
        for item in [c for c in candidates if c["Property"] == prop]:
            ...
            direct_actual_support = (
                ...
                # A direct lexical/domain gate can rescue a modest generic-NLI under-score,
                # but never an essentially unsupported score.
                and pos_nli >= 0.25
                and nli_margin >= 0.05
            )
        ...
        if not evaluated:
            results.append({
                ...
                "Evidence Source": "",
                "Evidence URL": "",
                "Decision Reason": "CE-18 v4.3 | No incident-relevant evidence candidate.",
            })
            continue
